...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const idioma = 'pt'; // is null? np the operator ?? fix that (i hope)
    ...
    if (textTrim || hasSupportedMedia) {
      ...
      if (!looksLikeCommand || isFinishMenu) {
        ...
        const assistantHandled = await tryMetajiAssistant(this, m).catch(() => false);
...
watchFile(file, async () => {
  ...
  const { sendMetajiExecutionLog } = await import('./src/libraries/metajiWebhook.js');
  void sendMetajiExecutionLog(global.conn, "Update 'handler.js'", {
    source: 'handler.js',
    event: 'file-update',
  }).catch(() => {});
