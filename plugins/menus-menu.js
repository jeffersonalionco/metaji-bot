...
const menuPt = require('../src/languages/pt/menus-menu.js.json');
const tradutor = menuPt.plugins.menu;

// Largura máxima do conteúdo por linha (para evitar quebra sem ┃ no WhatsApp)
const BOX_WIDTH = 28;
const PREFIX = '┃ ';

function ensureBoxBorder(text) {
  ...
  for (const line of lines) {
    if (line.startsWith('┃')) {
      ...
      if (content.length <= BOX_WIDTH) {
        ...
        // Quebra o texto manualmente e adiciona ┃ no início de cada linha
        let remaining = content;
        while (remaining.length > 0) {
          if (remaining.length <= BOX_WIDTH) {
            result.push(PREFIX + remaining);
            break;
          }
          // Prefere quebrar no último espaço antes de BOX_WIDTH
          const chunk = remaining.slice(0, BOX_WIDTH);
          const lastSpace = chunk.lastIndexOf(' ');
          const breakAt = lastSpace > 0 ? lastSpace : BOX_WIDTH;
          const lineContent = remaining.slice(0, breakAt).trimEnd();
          result.push(PREFIX + lineContent);
...
// Mapeamento: o que o usuário digita -> tag interna
const CATEGORIA_ALIASES = {
    'jogos': 'game', 'game': 'game', 'games': 'game',
    'download': 'downloader', 'downloads': 'downloader', 'downloader': 'downloader',
    'owner': 'owner', 'dono': 'owner', 'propietarios': 'owner', 'proprietarios': 'owner',
    'grupos': 'group', 'group': 'group', 'grupo': 'group',
    'ferramentas': 'tools', 'tools': 'tools', 'tool': 'tools',
    'stickers': 'sticker', 'sticker': 'sticker',
    'conversores': 'converter', 'converter': 'converter',
    'pesquisas': 'search', 'search': 'search', 'busca': 'search',
    'imagens': 'img', 'img': 'img', 'imagem': 'img', 'wallpaper': 'img',
    'info': 'info', 'informação': 'info', 'informacao': 'info',
    'xp': 'xp',
    'jadibot': 'jadibot', 'subbots': 'jadibot', 'subbot': 'jadibot',
    'efeitos': 'effects', 'effects': 'effects', 'effect': 'effects',
    'adultos': 'nsfw', 'nsfw': 'nsfw', 'adulto': 'nsfw',
    'random': 'random', 'aleatório': 'random', 'aleatorio': 'random',
    'maker': 'maker', 'criador': 'maker', 'logos': 'maker',
    'música': 'music', 'music': 'music', 'musica': 'music',
    'anime': 'anime', 'animes': 'anime'
};

const handler = async (m, { conn, usedPrefix, __dirname, isPrems, args }) => {
    ...
    try {
        ...
        const locale = localeMap['pt'] || 'pt-BR';
        ...
        try {
            ...
            week = d.toLocaleDateString('pt-BR', { weekday: 'long' });
        ...
        let role = user.role ? user.role : 'Novo';
        ...
        const defaultMenu = {
            before: ensureBoxBorder((tradutor.menu_header || '')
                .replace('@username', username)
                .replace('@author', global.author || 'Desconhecido')
                ...
                .replace('@pluginsCount', pluginsCount)),
            
            user_info: '\n' + (tradutor.user_info || '')
                .replace('@level', level)
                .replace('@exp', exp)
                .replace('@role', role || 'Novo')
                .replace('@limit', limit)
                .replace('@money', money)
        ...
        const categoriaArg = (args && args[0] || '').toLowerCase().trim();
        const tagFiltro = !categoriaArg ? null : (CATEGORIA_ALIASES[categoriaArg] || (Object.keys(tags).includes(categoriaArg) ? categoriaArg : null));

        const formatCommand = (cmdStr, limit) => {
            const fullCmd = cmdStr.replace(/%p/g, usedPrefix);
            const parts = fullCmd.trim().split(/\s+/);
            const base = parts[0] || fullCmd;
            const args = parts.slice(1).join(' ');
            const limitTag = limit ? ' ⭐' : '';
            return args ? `*• ${base}* _${args}_${limitTag}` : `*• ${base}*${limitTag}`;
        };

        const buildCategorySection = (tag) => {
            let pluginCommands = help.filter(menu => menu.tags && menu.tags.includes(tag) && menu.help).map(menu => {
                return menu.help.map(h => {
                    const cmdStr = menu.prefix ? h : `${usedPrefix}${h}`;
                    return formatCommand(cmdStr, menu.limit);
                }).join('\n')
            });
            ...
            if (extrasCommands[tag]) {
                ...
                pluginCommands.forEach(cmdGroup => {
                    cmdGroup.split('\n').forEach(line => {
                        ...
                        if (m) existingCommands.add(m[1]);
                    });
                });
                
                const filteredExtras = extrasCommands[tag].filter(extraCmd => {
                    const baseCmd = extraCmd.split(' ')[0];
                    return !existingCommands.has(baseCmd);
                });
                
                if (filteredExtras.length > 0) {
                    categoryCommands.push(
                        ...filteredExtras.map(cmd => formatCommand(cmd, false))
                    );
                }
            }
            
            if (categoryCommands.length === 0) return '';
            const hasLimit = categoryCommands.some(c => c.includes('⭐'));
            const footer = (hasLimit ? tradutor.section_footer_limit : tradutor.section_footer) || tradutor.section_footer || '';
            return categoryCommands.join('\n') + footer;
        };

        let _text;

        if (!categoriaArg) {
            const categoriasAtivas = Object.keys(tags).filter(tag => {
                const section = buildCategorySection(tag);
                return section !== '';
            });
            const listaComandos = categoriasAtivas.map((tag) => {
                const alias = Object.keys(CATEGORIA_ALIASES).find(a => CATEGORIA_ALIASES[a] === tag) || tag;
                return `*• ${usedPrefix}menu* _${alias}_`;
            }).join('\n');
            _text = [
                before + readMore,
        ...
        } else if (tagFiltro === null) {
            _text = [before + readMore, '\n\n╭━━ ❌ NÃO ENCONTRADO ━━\n\n"' + categoriaArg + '" não existe.\nUse ' + usedPrefix + 'menu para ver as categorias.', after].join('\n');
        } else {
            const section = buildCategorySection(tagFiltro);
            const submenuBefore = defaultMenu.before + readMore;
            _text = section 
                ? [submenuBefore, '\n\n' + section, after].join('\n')
                : [submenuBefore, '\n\n╭━━ 📭 VAZIO ━━\n\nNenhum comando nesta categoria.', after].join('\n');
        }
        ...
        let pp;
        const imageMap = {'pt': global.imagen1, 'es': global.imagen1, 'en': global.imagen4, 'ar': global.imagen5 };
        
        pp = imageMap['pt'] || global.imagen1;
