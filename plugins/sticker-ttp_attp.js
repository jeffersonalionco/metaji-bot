...
const handler = async (m, { conn, text, args, usedPrefix, command }) => {
 if (!text) throw `*[❗] Insira um texto*\n\n*—◉ Exemplo:*\n*◉ ${usedPrefix + command} Mystic-Bot*`;
 if (command == "attp") {
   const a1 = await (await fetch(`https://api.cafirexos.com/api/maker/attp?text=${decodeURI(text)}`)).buffer();
 ...
 if (command == "ttp2") {
   ...
   const stiker = await createSticker(ttppredd, false, global.packname, global.author, 20);
   m.reply(stiker);
 }
 if (command == "ttp") {
   const tctctctc = `${global.BASE_API_DELIRIUS}/canvas/ttp?text=${encodeURI(text)}`;
   const stiker = await createSticker(tctctctc, false, global.packname, global.author, 20);
   m.reply(stiker);
 }
};
