...
const TRADUCAO = {
  promote: {
    header: "*Recentemente um membro foi promovido a administrador*",
    group: "*◦ Grupo:* @group",
    new_admin: "*◦ Novo admin:* @user",
    executed_by: "*◦ Executado por:* @user"
  },
  demote: {
    header: "*Recentemente um administrador foi rebaixado a membro*",
    group: "*◦ Grupo:* @group",
    removed_admin: "*◦ Removido:* @user",
    executed_by: "*◦ Executado por:* @user"
  },
  member_add: {
    header: "*Recentemente um novo membro foi adicionado*",
    group: "*◦ Grupo:* @group",
    added_user: "*◦ Adicionado:* @user",
    added_by: "*◦ Adicionado por:* @user",
    self_added: "*◦ Entrou:* @user"
  },
  member_remove: {
    header: "*Recentemente um membro foi removido do grupo*",
    group: "*◦ Grupo:* @group",
    removed_user: "*◦ Removido:* @user",
    removed_by: "*◦ Removido por:* @user",
    self_removed: "*◦ Saiu:* @user"
  },
  group_settings: {
    header: "*Recentemente as configurações do grupo foram alteradas*",
    group: "*◦ Grupo:* @group",
    action: "*◦ O grupo foi:* @action",
    executed_by: "*◦ Executado por:* @user"
  },
  group_name: {
    header: "*Recentemente o nome do grupo foi alterado*",
    new_name: "*◦ Novo nome:* @name",
    executed_by: "*◦ Executado por:* @user"
  }
};
...
export async function before(m, { conn, participants }) {
  ...
  const safeOperation = async (operation, fallback = null) => {
    try {
      ...
      console.error('[DETECT-EVENTS] Erro na operação:', error);
      return fallback;
    }
  };
  ...
  try {     
    ...
    const tradutor = TRADUCAO;

    let groupName = "o grupo";
    let groupMetadata = groupMetadataCache.get(m.chat);
    let pp = 'https://raw.githubusercontent.com/GroupMetaJI/TheMystic-Bot-MD/master/src/avatar_contact.png';
    let img = null;
    ...
    if (!groupMetadata) {
      ...
      if (groupMetadata) {
        ...
        groupName = groupMetadata.subject || "o grupo";
    ...
    if (chat?.detect2) {
      switch (m.messageStubType) {
        ...
        case 26:
          await safeOperation(async () => {
            const accion = resolvedStubParameters[0]?.split('@')[0] === 'on' ? 'fechado' : 'aberto';
