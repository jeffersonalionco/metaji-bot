...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`_*< BAIXAR - SPOTIFY />*_\n\n*[ ℹ️ ] Digite o título da música do Spotify.*\n\n*[ 💡 ] Exemplo:* _${usedPrefix + command} Good Feeling - Flo Rida_`);

  try {
    let songInfo = await spotifyxv(text);
    if (!songInfo.length) return m.reply(`_*< BAIXAR - SPOTIFY />*_\n\n*[ ℹ️ ] Nenhuma música encontrada.*`);
    let song = songInfo[0];
    ...
    if (!data?.download) return m.reply(`_*< BAIXAR - SPOTIFY />*_\n\n*[ ℹ️ ] Ocorreu um erro. Por favor, tente novamente mais tarde.*`);

    let spotifyi = ` _*< BAIXAR - SPOTIFY />*_\n\n`;
    spotifyi += ` ▢ *Título:* ${data.title}\n`;
    spotifyi += ` ▢ *Artista:* ${data.artist}\n`;
    spotifyi += ` ▢ *Álbum:* ${song.album}\n`;
    spotifyi += ` ▢ *Duração:* ${data.duration}\n\n`;
    spotifyi += `> *[ ℹ️ ] Enviando o áudio, aguarde...*`;
...
const handler = async (m, { conn, text }) => {
 if (!text) throw `*[❗] Ingrese el nombre de alguna canción de spotify.*`;
  try {
    ...
    let letra;
    letra = `${letra_s ? letra_s + '\n\n🤴🏻 Descarga por Group MetaJI & TheMystic-Bot-MD 🤖' : '🤴🏻 Descarga por Group MetaJI & TheMystic-Bot-MD 🤖'}`  
    const tags = {
