...
const handler = async (m, {conn, usedPrefix, usedPrefix: _p, __dirname, text, isPrems}) => {
  const textos = [
    '*[❗] Comandos +18 estão desativados neste grupo. Se você for admin e quiser ativar, use*',
    '*Olá*',
    '*< MENU+18 />*',
    '*[❗] Este menu está com um erro interno e não foi possível enviá-lo. Reporte ao proprietário do bot.*',
  ];

if (!db.data.chats[m.chat].modohorny && m.isGroup) throw `${textos[0]} ${usedPrefix}enable modohorny*`; 
  try {
    const pp = imagen5;
