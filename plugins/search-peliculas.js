...
const handler = async (m, {text, conn}) => {
 try {
  if (!text) throw `*[❗] Digite o nome de algum filme para buscar.*`;
  let aaaa;
  let img;
  ...
  const res = await aaaa.map((v) => `*🎬 • Nome:* ${v.title}\n*🍿 • Url:* ${v.link}`).join`\n\n─────────────────\n\n`;
  const ads = `*💫 Bloqueador de anúncios recomendado:* Block This\n*⛨ • Link:* https://block-this.com/block-this-latest.apk\n\n≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣\n\n`;
  conn.sendMessage(m.chat, {image: {url: img}, caption: ads + res}, {quoted: m});
 } catch {
   return conn.sendMessage(m.chat, {text: '*[❗] Erro, nenhum resultado obtido.*'}, {quoted: m});
 }
};
