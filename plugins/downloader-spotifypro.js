// TheMystic-Bot-MD - Group MetaJI - descargas-spotifypro.js
...
const handler = async (m, { conn, text }) => {
  if (!text) throw `*[❗] Digite o link de um track, playlist ou álbum do Spotify ou simplesmente o nome de uma música para buscar.*`;
  const isSpotifyUrl = text.match(/^(https:\/\/open\.spotify\.com\/(album|track|playlist)\/[a-zA-Z0-9]+)/i);
  if (!isSpotifyUrl && !text) throw `*[❗] Digite o link de um track, playlist ou álbum do Spotify.*`;
  try {
     if (isSpotifyUrl) {
      if (isSpotifyUrl[2] === 'album') {
        ...
        let spotifyi = `*• 💽 Spotify Download •*\n\n`
          spotifyi += `	*Álbum:* ${album.metadata.title}\n`
          spotifyi += `	◦  *Artista:* ${album.metadata.artists}\n`
          spotifyi += `	◦  *Publicado:* ${album.metadata.releaseDate}\n`
          spotifyi += `	◦  *Total de tracks:* ${album.trackList.length}\n\n`
          spotifyi += `Os áudios estão sendo enviados, aguarde um momento..`
        ...
        let spotifyi = `*• 💽 Spotify Download •*\n\n`
          ...
          spotifyi += `	◦  *Álbum:* ${track.album.name}\n`
          spotifyi += `	◦  *Publicado:* ${track.album.releasedDate}\n\n`
          spotifyi += `O áudio está sendo enviado, aguarde um momento..`
        ...
        let spotifyi = `*• 💽 Spotify Download •*\n\n`
          ...
          spotifyi += `	◦  *Total de tracks:* ${tracks.length}\n\n`
          spotifyi += `Os áudios da playlist estão sendo enviados, aguarde um momento..`
...
for (let i = 0; i < tracks.length; i++) {
    ...
    await conn.sendMessage(target, { audio: track.audioBuffer, fileName: `${tracks[i].track.name}.mp3`, mimetype: 'audio/mpeg' }, { quoted: m });
        ...
        let spotifyi = `*• 💽 Spotify Download •*\n\n`
          ...
          spotifyi += `	◦  *Álbum:* ${searchTrack.album.name}\n`
    ...
    console.error(error);
    throw `*[❗] Erro, não foram encontrados resultados.*`;
  }
};
