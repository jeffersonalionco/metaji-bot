...
const handler = async (m, {conn, args, usedPrefix, command}) => {
 ...
 if (args.length >= 1) {
   ...
   text = m.quoted.text;
 } else throw '*[❗] Uso incorreto do comando, adicione um texto*';
 if (!text) return m.reply('*[❗] Uso incorreto do comando, adicione um texto*');
 const who = await m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender;
 const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
 const mishi = text.replace(mentionRegex, '');
 if (mishi.length > 30) return m.reply('*[❗] O texto não pode ter mais de 30 caracteres*');
