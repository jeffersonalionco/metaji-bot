...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  if (!text) throw `*[❗] Digite um link do X (Twitter), exemplo: ${usedPrefix + command}* https://twitter.com/auronplay/status/1586487664274206720?s=20&t=3snvkvwGUIez5iWYQAehpw`;
if (enviando) return;
    enviando = true;
try {
 ...
 if (res?.result.type == 'video') {
     const caption = res?.result.caption ? res.result.caption : '*Aqui está sua imagem*';
...
const TwitterDL = async (url) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message: "Houve um erro ao obter o ID do Twitter. Certifique-se de que sua URL do Twitter está correta!",
      });
