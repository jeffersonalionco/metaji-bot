...
const handler = async (m, {usedPrefix, conn}) => {
  try {
    ...
    if (user.health < 80) {
      return conn.reply(
          m.chat,
          `_${htki} Baixa Saúde ${htka}_\n\nSua saúde 💔 está abaixo de *80!!* Por favor cure-se antes de aventurar novamente`,
          m,
      );
    }
    if (new Date() - user.lastadventure <= cooldown) {
      return conn.reply(
          m.chat,
          `${htki} Descansando ${htka}\n\nVocê já aventurou, aguarde o tempo de descanso\n\n⏱️ ${timers.toTimeString()} 🛫 Você está aventurando em `,
          m,
      );
    }
    const rewards = reward(user);
    let text = `🛫 Você está aventurando em *» ${kt[1][0].name}*
...
${cmenua}`;
    ...
    conn.reply(m.chat, `${htki} Aventura ${htka}\n\n${text}`, m);
    user.lastadventure = new Date() * 1;
  } catch {
    conn.reply(
        m.chat,
        '*[❗] Ocorreu um erro, tente novamente. Provavelmente a API não gerou a imagem*',
        m,
    );
  }
};
