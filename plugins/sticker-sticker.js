...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `*[❗] Responda a um vídeo, imagem ou insira o link de uma imagem com terminação .jpg que será convertida em sticker. Use o comando ${usedPrefix + command}*`;
      let out;
      try {
      ...
      else return m.reply(`*[❗] O link/URL não é válido. A terminação do link deve ser .jpg. Exemplo: ${usedPrefix}s https://telegra.ph/file/0dc687c61410765e98de2.jpg*`);
    }
  } catch (e) {
    console.error(e);
    if (!stiker) stiker = e;
   ...
   else return m.reply(`*[❗] Arquivo multimídia faltando. Responda a um vídeo ou imagem que será convertido em sticker, ou use o comando: ${usedPrefix + command}*`);
  }
};
handler.help = ['sfull'];
