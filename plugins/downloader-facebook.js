...
let handler = async (m, { args, conn, text, usedPrefix, command }) => {
  if (!text) throw `_*< BAIXAR - FACEBOOK />*_\n\n*[ ℹ️ ] Digite um link do Facebook.*\n\n*[ 💡 ] Exemplo:* ${usedPrefix + command} https://www.facebook.com/share/v/1E5R3gRuHk/`;
    ...
    try {
        ...
        if (links.length === 0) return await conn.sendMessage(m.chat, { text: '*[ ❌ ] Não foram encontrados links de download.*' }, { quoted: m });

        let download = getDownloadLink(platform, links);

        if (!download) return await conn.sendMessage(m.chat, { text: '*[ ❌ ] Erro ao obter o link de download.*' }, { quoted: m });

        if (Array.isArray(download)) {
            for (const media of download) {
                try {
                    await conn.sendMessage(m.chat, { image: { url: media }, caption: `*[📥] Downloader*\n${media}` }, { quoted: m });
                } catch (err) {
            ...
            try {
                ...
                const caption = `*📥 Download do ${platform} concluído!*`;
                ...
                return await conn.sendMessage(m.chat, { text: `*[❌] Erro ao enviar o arquivo:* ${err.message}` }, { quoted: m });
            }
        }

    } catch (error) {
        console.log('Error en downloader:', error);
        return await conn.sendMessage(m.chat, { text: `*[❌] Ocorreu um erro:*\n${error.message || error}` }, { quoted: m });
    }
};
...
async function fetchDownloadLinks(text, platform, conn, m) {
    ...
    if (!html || res?.data?.status !== 'success') {
        await conn.sendMessage(m.chat, { text: '*[❌] Erro ao obter dados do servidor.*' }, { quoted: m });
        return null;
    }
