...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw '*[❗𝐈𝐍𝐅𝐎❗] Como usar este comando?*\n—◉ _#logo (efeito) (texto)_\n*Exemplo:*\n—◉ _#logo 3d-deep-sea-metal Mystic_\n\n*[❗] Quando pedir mais texto, use:*\n—◉ _#logo (efeito) (texto1|texto2)_\n*Exemplo:*\n—◉ _#logo Wolf-Logo-Galaxy Mystic|Bot_\n\n*<Lista de efeitos/>*\n\n° ඬ⃟📝 #logo ' + effects.map((v) => v.title).join('\n° ඬ⃟📝 #logo ');
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw '*[❗] Falta o texto para realizar o logo*' })
   if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] O efeito ${effect} não está na lista de efeitos*` : `*[❗𝐈𝐍𝐅𝐎❗] Uso correto: ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: `*Pegue sua imagem personalizada!!*\n*Efeito: ${effect}*`}, {quoted: m});
};
handler.help = ['logos'];
handler.tags = ['maker'];
