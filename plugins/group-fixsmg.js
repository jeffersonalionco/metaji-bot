...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  try {
    ...
    if (filesDeleted === 0) {
      await conn.sendMessage(m.chat, {text: '*[❗] Nenhum arquivo foi encontrado que inclua o ID do chat*'}, {quoted: m});
    } else {
      await conn.sendMessage(m.chat, {text: `*[❗] Foram removidos ${filesDeleted} arquivos de sessão*`}, {quoted: m});
    }
  } catch (err) {
