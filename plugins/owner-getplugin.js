...
const handler = async (m, {conn, isROwner, usedPrefix, command, text}) => {
  ...
  if (!text) throw `*[❗] Digite o nome de algum plugin (arquivo) existente.*\n\n*—◉ Exemplo*\n*◉ ${usedPrefix + command}* info-infobot\n\n*—◉ Lista de plugins (arquivos) existentes:*\n*◉*${ar1.map((v) => ' ' + v).join`\n*◉*`}`;
  if (!ar1.includes(text)) return m.reply(`*[❗] O plugin "${text}" não foi encontrado. Lista de plugins existentes:*\n*◉*${ar1.map((v) => ' ' + v).join`\n*◉*`}`);
  let o;
  try {
...
handler.help = ['getplugin'].map((v) => v + ' *<nome>*');
handler.tags = ['owner'];
